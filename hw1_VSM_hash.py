    ...
    def __init__(self, dics, words):
        ...
        for dic in self.dics:
            ...
            for key in keys:
                ...
                v[n] = self.tf(dic, key, 'Raw')*self.idf(key, 'smooth')
            self.d_vs.append(v)
            count = count+1
            if(count%50)==0:
                print(count)
...
answer = open('K:\AcadamicFiles\資訊檢索\hw1\\answer_hash.txt','w')
...
for i in range(len(qs_ids)):
    ...
    #print(query)
    que = query.split(' ')
    for wd in range(len(que)):
        que[wd] = word_hash(que[wd])
    query = ''
    for s in que:
        query = query + str(s)+' '
    query = query.strip()
